...
def graficar():
    ...
    datax=np.linspace(x1.get() , x1.get() + ((x2.get()-x1.get())*punt.get()) , 69)
    ...
    canvas.draw()
    botonG.place(x=aio,y=aio)
    botonG1.place(x=30,y=220)

def graficar1():
    global datax,datay
    line, = ax.plot(datax, datay, color = rgb ,linewidth=5)
    canvas.draw()

    datax=np.linspace(x1.get(), h1*punt.get() , 69)
    if (punt.get()>=2):datay = y1.get()+(c1*(datax+x11))
    if (punt.get()>=3):datay += c2*(datax+x11)*(datax+x21)
    if (punt.get()>=4):datay += c3*(datax+x11)*(datax+x21)*(datax+x31)
    if (punt.get()>=5):datay += c4*(datax+x11)*(datax+x21)*(datax+x31)*(datax+x41)
    ...
    ax.axhline(linewidth=2, color=letra)
    ax.axvline(linewidth=2, color=letra)
    line, = ax.plot(datax, datay, color ='r')
    canvas.draw()

from tkinter import Canvas

rgb="#3A3A3A" # Cambiar color de fondo y letra
letra="white"
...
botonG1 = Button(ventana, text='Graficar', width = 15, command= graficar1)


ventana.mainloop() # Para que no se cierre la ventana 
pass